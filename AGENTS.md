...
Codex is the Tier 1, first-class runtime; normal model/provider selection comes from effective Codex configuration, while supported engineering workflows resolve child-spawn reasoning through `protocols/REASONING_POLICY.md`. Generated role profiles may override only the model/provider. The bounded exception is `protocols/CAPABILITY_RECOVERY.md`: after effort-first recovery and a repeated material reasoning failure, an eligible `executor` or `generalist` child may receive one temporary profile-approved higher tier.
