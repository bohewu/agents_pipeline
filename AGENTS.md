...
For Codex custom-role dispatch, use `fork_turns = none` whenever the spawn selects a registered role or non-parent model/reasoning configuration; a full-history fork inherits the parent role/model and is not valid evidence that a workspace profile was applied.
