...
- Codex CLI 0.145.0 or newer for the managed multi-agent V2 feature flag and per-spawn reasoning selector
- A valid local login
...
Codex CLI 0.145.0 remains the minimum feature baseline for the repository's managed multi-agent V2 contract. For current GPT-6 Sol/Luna model-selection workflows and routing measurements, Codex CLI 0.156.1 or newer is the recommended operational baseline. The newer recommendation is not a claim that every repository feature requires 0.156.1.
...
For the default global Codex target (`~/.codex`), the installer publishes 16 managed skills under `~/.agents/skills/`: eleven formal workflow skills and five reusable capability skills. Each carries `.agents-pipeline-skill.json` with a content digest; updates are rollback-capable and replace each directory atomically. Unowned, corrupt-marker, linked, or junction-backed targets cause a safe refusal, while modified marker-owned copies are preserved outside the discovery root before repair. `$run-adaptive` is skill-only and selects an existing Simple, Flow, or Pipeline workflow without creating a role. A custom/test Codex home publishes skills only when `--user-skills-root` / `-UserSkillsRoot` is supplied. Direct workspace materialization never installs user skills, and there is no `run-goal` skill.
...
Codex activates the workspace layer only when its effective project trust permits it. Profile setup never grants trust; workspace status reports file `health`, `project_trust`, and `profile_eligibility` independently. For a linked Git worktree without its own trust entry, the profile manager asks Codex `config/read` whether that worktree's `.codex` layer is enabled. Codex 0.156.1 may use the verified main Git checkout as the trust identity; an unavailable or inconclusive read remains `unknown` and ineligible. Eligibility describes the trust gate, while Codex remains authoritative for full effective-config parsing and routing.

Treat every Codex worktree as its own effective workspace for profile verification. Do not assume that a source checkout's trust state, managed `.codex/config.toml`, role bindings, or profile eligibility applies to a newly created worktree. Codex resolves a linked worktree's effective trust through its own Git metadata checks; this does not copy the source checkout's profile. Before managed dispatch there, run the installed workspace `status --json` flow and verify the same health, effective trust, catalog, and worktree-local binding conditions used for an ordinary checkout.

For a new disposable detached worktree used by a Web session agent Job, `health = ok`, `catalog_state = inherit`, `profile_eligibility = not_configured`, `project_trust = unknown`, and absent local role TOML are a normal initial state. When source profile reuse is required, obtain its profile/model-set from a verified source snapshot, set only the disposable worktree, and recheck for `health = ok`, `catalog_state = current`, `profile_eligibility = eligible`, `project_trust = trusted`, and `configuration_compatibility = current` before inspecting local bindings. Guard optional file probes against expected absence. The source Project Session does not become the new worktree Project's recording Session; use the bootstrap-returned Session, and re-observe an uncertain bootstrap before creating anything else. Project/session mismatch remains fail-closed. WebCodex `run_process` accepts a native executable and literal argv; shell syntax belongs in `run_shell` or a script surface, including `bash -lc` and `sh -c` command modes. A pre-start interface rejection is not a Job dispatch.

Manifest-backed `$run-*` skills adopt the globally installed workflow definition and never manually load raw repository roles; `$run-adaptive` selects among the three installed engineering definitions in place, then maps its route-independent preset/policy to that workflow. Every preset remains Simple-eligible through a bounded Adaptive wrapper, while Flow and Pipeline use native controls. Before dispatch, their workspace-profile preflight fails closed when status cannot be verified or file `health` is not `ok`; Adaptive prompt-only generation warns without dispatching. A healthy but ineligible profile warns and safely uses global routing. After that gate, effective Codex configuration can apply an eligible workspace profile to dispatched role names, preserving project-specific resource routing without a project-local skill copy. Managed `use <mode>` forms remain compatibility aliases for manifest-backed modes and follow the same preflight.
...
Codex exporters omit machine-wide limits. The full/global installer ensures `agents.max_concurrent_threads_per_session = 8` and `agents.max_depth = 1`, preserves explicit new values, and migrates numeric legacy `agents.max_threads` values. Codex 0.145.0 uses the new concurrency key but ignores `agents.max_depth` on V2. Workspace configs inherit the global values. Formal skills keep primary workflow control in the current/main agent, and exported subagent roles are leaf workers so they cannot create nested dispatches.
...
Codex named workspace profiles use the sole built-in `openai@4` / `openai-gpt6-v1@1` catalog/projection pair, mapping GPT-6 Luna/Sol/Astra to mini/standard/strong. A workspace manifest records its profile, mapping, projection, and resolved bindings. Previously saved `openai@3`, LSA v1/v2, and `openai-legacy@2` overlays are diagnosed as pinned or retired and require an explicit workspace `set --model-set openai` before new dispatch; they cannot hot-reload a running workflow or resume under a different identity. `clear` restores parent-session inheritance. See [runtime profile details](docs/runtime-agent-model-profiles.md#versioned-codex-model-sets) for commands and the `low` limits.
...
After upgrading Codex, verify the active runtime path before relying on routing claims. A successful spawn is not proof that the expected role, model, or reasoning effort ran; confirm a representative managed child with the existing trace helper. When the Codex installation uses a daemon-backed execution path, also verify that the runtime serving the session reflects the intended upgrade rather than assuming the front-end CLI version alone proves it.
