...
An external orchestrator with an MCP/Runner that can read the installed skill and workflow definition and execute local commands may use the [experimental external Simple entry](docs/external-simple-orchestration.md). The MCP brand does not select routing or grant trust. That entry uses independently verified `codex exec` leaves through the external dispatcher, requires each worktree's own eligible OpenAI profile, and does not claim native managed-child identity or full `$run-*` parity. A connector without local process access cannot use this entry.
