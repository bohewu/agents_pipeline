...
Treat every Codex worktree as its own effective workspace for profile verification. Do not assume that a source checkout's trust state, managed `.codex/config.toml`, role bindings, or profile eligibility applies to a newly created worktree. Before managed dispatch there, run the installed workspace `status --json` flow and verify the same health, trust, catalog, and binding conditions used for an ordinary checkout.
