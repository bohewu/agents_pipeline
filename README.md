...
```powershell
$tag = "v0.36.2"; Invoke-WebRequest "https://raw.githubusercontent.com/bohewu/agents_pipeline/$tag/scripts/bootstrap-install-codex.ps1" -OutFile .\bootstrap-install-codex.ps1; pwsh -NoProfile -File .\bootstrap-install-codex.ps1 -Version $tag -Target "$HOME\.codex"
```

macOS/Linux (Bash):

```bash
tag="v0.36.2" && curl -fsSL -o ./bootstrap-install-codex.sh "https://raw.githubusercontent.com/bohewu/agents_pipeline/${tag}/scripts/bootstrap-install-codex.sh" && bash ./bootstrap-install-codex.sh --version "${tag}" --target "$HOME/.codex"
```
...
```powershell
$tag = "v0.36.2"; Invoke-WebRequest "https://raw.githubusercontent.com/bohewu/agents_pipeline/$tag/scripts/bootstrap-install-claude.ps1" -OutFile .\bootstrap-install-claude.ps1; pwsh -NoProfile -File .\bootstrap-install-claude.ps1 -Version $tag -Target "$HOME\.claude\agents"
```

macOS/Linux (Bash):

```bash
tag="v0.36.2" && curl -fsSL -o ./bootstrap-install-claude.sh "https://raw.githubusercontent.com/bohewu/agents_pipeline/${tag}/scripts/bootstrap-install-claude.sh" && bash ./bootstrap-install-claude.sh --version "${tag}" --target "$HOME/.claude/agents"
```
...
```powershell
$tag = "v0.36.2"; Invoke-WebRequest "https://raw.githubusercontent.com/bohewu/agents_pipeline/$tag/scripts/bootstrap-install-copilot.ps1" -OutFile .\bootstrap-install-copilot.ps1; pwsh -NoProfile -File .\bootstrap-install-copilot.ps1 -Version $tag -Target "$HOME\.copilot\agents"
```

macOS/Linux (Bash):

```bash
tag="v0.36.2" && curl -fsSL -o ./bootstrap-install-copilot.sh "https://raw.githubusercontent.com/bohewu/agents_pipeline/${tag}/scripts/bootstrap-install-copilot.sh" && bash ./bootstrap-install-copilot.sh --version "${tag}" --target "$HOME/.copilot/agents"
```

Release invariant: `VERSION=0.36.2` must release as `v0.36.2`.
...
ProblemSpec/DevSpec 1.1 records whether blocking acceptance criteria come from the explicit user request, an existing repository contract, or evidenced necessary compatibility. Specs, plans, TaskLists, Definitions of Done, test plans, and review findings remain derivative: repeating an assumption or workflow suggestion does not turn it into original scope. Generated validation defaults to `workflow_suggested`; it may expose a real product defect but cannot block merely because the suggested check is absent.

Product tests and fixtures are ordinary verification. Validation infrastructure such as harness frameworks, validator generators, certification wrappers, test orchestrators, and proof machinery is forbidden by default. A TaskList or FlowTaskList must record prior explicit-user authorization or an independently established repository contract that predates the workflow; same-run artifacts, executors, failed harnesses, retries, recovery, and reviewers cannot add that authority without separate user approval. Legacy 1.0 payloads remain usable: omitted authorization means false, and the orchestrator reconciles missing provenance against the original request or pre-workflow repository evidence.
...
- [Adaptive reasoning and model-capability roadmap](docs/adaptive-reasoning-and-model-routing-roadmap.md)
