...
- Single source of truth: root `VERSION` file (SemVer without `v`, for example `0.11.3`).
- Use SemVer tags with `v` prefix (for example: `v0.11.3`).
...
- Release CI checks `VERSION` and tag alignment (`VERSION=0.11.3` must release as `v0.11.3`).
...
- Trigger: push tag `v*` (for example `v0.11.3`) or manual `workflow_dispatch`
- Output assets:
