...
- Do NOT expand scope
- Preserve requirement authority: generated specs, plans, tasks, DoD, test plans, and reviews are derivative and cannot promote assumptions or workflow suggestions into original requirements
- Validation infrastructure is forbidden unless an `explicit_user` or `existing_contract` source authorized it before dispatch
...
- Contract format:
  - When provided as structured JSON, the payload SHOULD conform to `protocols/schemas/modernize-exec-handoff.schema.json`.
...
- Validate it against `protocols/schemas/modernize-exec-handoff.schema.json` when runtime support exists.
...
1. Call `node tools/status-event.js --event stage.completed --payload-json '<json>'` so the runtime-neutral status writer can write/update `<run_output_dir>/checkpoint.json` with:
...
Stage-scoped roles without a canonical task use an actual `task_intent`; only a
legacy handoff with neither intent nor class may use the role-target fallback.
Canonical task attempts pass `task_intent`, `intent_baseline_class`,
`classification_source`, legacy-compatible `reasoning_class`, and
`reasoning_signals` from TaskList or DeltaTaskList. Stage 6 uses
`dispatch_context = pipeline-review`; when `review_reasoning_effort = max`,
...
conflicts. `--review=max` remains ordinary deep review, does not certify work,
...
Stage 0: @specifier -> source-aware `problem-spec.json` using schema version 1.1; assumptions cannot become blocking acceptance criteria
Optional Stage 0.5: if DevSpec policy matches, call @specifier again to produce source-aware `dev-spec.json` with validation authority, then call @doc-writer to render a Markdown artifact from the same contract and persist its content to the canonical path `dev-spec.md`
...
Stage 3: @atomizer -> `task-list.json` (atomic DAG) using ProblemSpec, PlanOutline, optional RepoFindings, and optional DevSpec; every new task must carry explicit `trace_ids` back to sourced acceptance criteria, with additional story/scenario/test ids when DevSpec exists; then emit `tasks.registered` for the canonical task set so the status writer can refresh `run-status.json` with `layout = expanded`, `task_list_path`, `task_counts`, and task refs
  - Every task must use policy v2 metadata: `task_intent`, matching `intent_baseline_class`, `classification_source = task_intent`, legacy-compatible `reasoning_class`, and bounded `reasoning_signals`, all selected independently from risk and complexity
  - `allow_degraded_deep` remains false unless an input contract explicitly authorizes that deep-only compatibility exception
  - Every new 1.1 task must carry `validation_infrastructure`; default it to `{ "authorized": false }`. True requires a prior `explicit_user` source or independently established `existing_contract` that predates this workflow, plus concrete `source_ref`; same-run artifacts cannot authorize it unless the user separately approves them
...
- For every task-worker handoff (`@executor`, `@peon`, `@generalist`, or `@doc-writer`), include an explicit bounded execution profile. Derive verification and repair rigor from task `risk` / `complexity`, while the independent reasoning policy uses `reasoning_class` / `reasoning_signals`:
  ...
  - include `validation_infrastructure` unchanged after legacy normalization; refuse only a task that would create or expand validation infrastructure without recorded authorization, not a legacy task merely missing the field
...
- Authority Gate: schema-1.1 acceptance criteria preserve `explicit_user`, `existing_contract`, or evidenced `necessary_compatibility` provenance; assumptions and workflow suggestions cannot seed blocking tasks.
- DevSpec Gate: If generated, stories/scenarios/test plan must remain aligned to ProblemSpec and usable by humans.
- Validation Infrastructure Gate: every task defaults to unauthorized; only prior user or repository-contract authority can permit infrastructure work, and execution/review cannot add that authority later.
