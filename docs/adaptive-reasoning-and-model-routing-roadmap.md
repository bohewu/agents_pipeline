...
The filename is retained for continuity. Policy v2 does **not** route models:
it classifies child work, validates the capability of the profile-selected
role/model tier, and selects only a child-spawn reasoning effort.
...
- Selecting a raw provider model identifier from a task, signal, or resolver.
- Changing the selected role model at dispatch time.
...
- Adding automatic model downgrades, upgrades, or dynamic model routing.
...
It must not turn observations into dynamic raw-model routing. If a selected
