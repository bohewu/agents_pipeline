...
- When the Codex spawn surface exposes `agent_type`, `model`, or `reasoning_effort`, select registered custom roles or non-parent model/reasoning configurations through native spawn selectors without full-history forks, then verify the spawned child trace. Full-history forks may inherit the parent's role and model configuration and are not proof of custom-role/profile routing.
- When reading the installed definition for Codex mode simulation, focus on mode behavior, task decomposition, delegation rules, and output style; ignore adapter details for other runtimes.
