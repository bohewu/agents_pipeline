...
The default global install to `~/.codex` also publishes exactly eleven formal `run-*` workflow skills to `~/.agents/skills/`. A custom global/test Codex home publishes skills only when `--user-skills-root` or `-UserSkillsRoot` is supplied. Projects do not need their own skill copy. Manifest-backed skills adopt the global workflow definition; `$run-adaptive` selects among the existing Simple, Flow, and Pipeline definitions without adding a role. Each skill runs the workspace-profile health gate before effective trusted Codex configuration may apply workspace-local model routing to dispatched roles. Direct workspace materialization never installs user skills.
...
On Codex surfaces that expose `agent_type`, `model`, or `reasoning_effort`, any spawn that selects a registered custom role or a non-parent model/reasoning configuration MUST use `fork_turns = none`. A full-history fork inherits the parent agent type, model, and reasoning effort; use it only when that inheritance is intentional. If the selectors are unavailable, do not claim that workspace profile routing succeeded.
...
- When the Codex spawn surface exposes `agent_type`, `model`, or `reasoning_effort`, selecting a registered custom role or a non-parent model/reasoning configuration requires `fork_turns = none`. Full-history forks intentionally inherit the parent's role and model configuration and are not a custom-role/profile-routing path.
- When reading the installed definition for Codex mode simulation, focus on mode behavior, task decomposition, delegation rules, and output style; ignore adapter details for other runtimes.
