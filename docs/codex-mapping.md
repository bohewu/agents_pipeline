...
The exporter intentionally omits `agents.max_threads` and `agents.max_depth`; machine-wide limits remain owned by the user's Codex configuration. Codex 0.145.0 uses `agents.max_threads` as the fallback for V2 concurrency when `features.multi_agent_v2.max_concurrent_threads_per_session` is absent. `agents.max_depth` limits V1 only and is ignored by V2.
...
Direct exporter output and normal global Codex installs both leave `agents.max_threads` and `agents.max_depth` runtime-owned. Installs preserve existing user values and boolean or object-form `features.multi_agent_v2` settings while forcing that feature enabled. Generated configs leave both agent-limit keys absent. Project profile overlays inherit the effective global values, and exported subagent roles enforce the leaf-worker boundary that V2 does not derive from `max_depth`.
