...
6. Verify the observed role and effective effort with `codex-child-trace.js`. Verify an expected model only when the eligible workspace profile proves one; otherwise report the bounded observed model without claiming profile routing.
