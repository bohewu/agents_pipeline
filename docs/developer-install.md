...
└─ <capability>/                reusable design and audit skills
tools/
...
The profile manager deliberately does not expose this materialization as a Claude/Copilot workspace profile. Call a direct installer explicitly when compatibility requires it. Direct Codex workspace materialization also does not install or change the global managed collection under `~/.agents/skills/`.
...
Codex ignores project `.codex/` config layers unless its effective project trust permits them. The manager does not change trust. In Codex 0.156.1, a linked worktree can use the verified main Git checkout's trust decision while keeping its own `.codex/` profile. If the worktree has no direct trust entry, the manager requires a positive Codex `config/read` proof that its own project layer is enabled; missing Codex, failed Git metadata validation, or failed read remains ineligible. For deterministic tests, establish trust through Codex, then assert workspace JSON status has `health: "ok"`, `project_trust: "trusted"`, and `profile_eligibility: "eligible"`. Use Codex `config/read` to prove effective role registration, then inspect a spawned child trace to prove its actual `agent_role` and model; eligibility and config readback alone do not prove runtime selection.

Global targets are `~/.codex`, `~/.claude/agents` with `~/.claude/CLAUDE.md`, and `~/.copilot/agents`. Codex global `status`/`clear` continue through the runtime installer only for diagnostics and legacy cleanup; global profile `set` is rejected. Claude Code and Copilot retain global `set`/`clear`. Full/global Codex installs default `agents.max_concurrent_threads_per_session` to `8` and `agents.max_depth` to `1`, preserve explicit new values, and migrate numeric legacy `agents.max_threads` values. Workspace Codex profiles inherit those effective global values. Codex 0.145.0 ignores `agents.max_depth` on V2; generated subagent role instructions enforce the direct-child boundary instead.
