...
- Expected: Preserve all seven full-handoff sections, aligned IDs and version markers, naming/output guidance, manual checks, external package, and the final Direct Use Prompt. The external package's reusable prompt record retains the same prompt ID/version and includes either the full non-empty prompt or an explicit reference to this response's Reusable Prompt or Direct Use Prompt for that same ID/version.
- Failure conditions: Substitutes prompt-only output; omits or renames required sections or identifiers; the external reusable prompt record contains only an ID, subject list, placeholder, or vague/absent reference; claims assets were generated.
...
- Current verification (2026-09-15): the installed v0.37.4 managed-role Sol/medium run read only the full reference, but its external reusable prompt record omitted both prompt text and a reference. After clarifying the shared contract, one isolated read-only Sol/medium CLI run with the exported role and checkout reference passed: all seven sections were ordered, IDs/version aligned, `prompt_reference` explicitly targeted the same response's Reusable Prompt for that ID/version, and reusable/direct prompt text matched. It read only the canonical full reference. This is candidate instruction-following evidence, not a post-install managed-role rerun; no image was generated or global installation changed.
