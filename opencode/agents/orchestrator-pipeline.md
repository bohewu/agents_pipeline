...
- After each task completion or reconciliation point, immediately flush the semantic status deltas needed for that point. Prefer one `status_runtime_event` call with `event = "batch"` when a task outcome and its related agent lifecycle deltas land together; use single-event calls only when there is exactly one delta or an intermediate write matters. Coalesce heartbeats so only the latest still-useful heartbeat per active agent is flushed. Apply the same rule to stage-scoped subagent dispatch/completion even when no canonical task exists yet.
