...
- Flow uses the shared `status_runtime_event` contract in `opencode/protocols/PIPELINE_PROTOCOL.md` for checkpoint/status writes, including batched same-run flushes via `event = "batch"` when appropriate and coarse standalone heartbeat cadence when no richer flush is needed.
