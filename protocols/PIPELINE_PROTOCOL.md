...
- `agent.started` / `agent.heartbeat` / `agent.finished`: add `agent_id`; include `agent` on start and `task_id` only when attached to a canonical task. Instrumented Flow/Pipeline attempts include the complete ReasoningDecision as `reasoning` and update it when effective-effort trace evidence becomes available. A verified independent `codex-external-role.py` root uses `external_dispatch_evidence` from that dispatch result; native managed children continue to use `trace_evidence`. Do not present an independent root as a native child trace or attach both evidence types.
