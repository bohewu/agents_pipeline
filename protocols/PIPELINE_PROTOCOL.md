...
- Simple, Flow, Pipeline, and Adaptive accept `--reasoning=inherit|shadow|adaptive`; policy v2 defaults to `adaptive`. `inherit` preserves classification metadata but never applies a selector, so exact overrides and strict assurance conflict. `shadow` computes without applying; strict assurance conflicts. `adaptive` requests the returned child selector and verifies local Codex trace evidence before claiming the effective-effort contract was enforced. Matching parent and child effort remains selector-causality indeterminate.
