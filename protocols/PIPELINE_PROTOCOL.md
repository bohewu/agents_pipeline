...
- Development verification classifies failures as product, harness, or operational before repair. Harness/operational failures never consume repair, workflow retry/re-dispatch, or capability-recovery budget; the same harness/infrastructure signature twice stops as a blocker. Validation tooling remains single-purpose and proportional, and fresh immutable certification is reserved for an explicit final contract after focused and required integration checks pass.
...
- Review also judges validation scope, cost, and necessity; an internally consistent but unnecessary validation framework is not a reason to expand delivery work.
...
- Simple, Flow, Pipeline, and Adaptive accept `--reasoning=inherit|shadow|adaptive`; policy v2 defaults to `inherit`. `inherit` preserves classification metadata but never applies a selector, so exact overrides and strict assurance conflict. `shadow` computes without applying; strict assurance conflicts. Explicit `adaptive` requests the returned child selector and verifies local Codex trace evidence before claiming the effective-effort contract was enforced. Matching parent and child effort remains selector-causality indeterminate.
...
- Observed effort below dispatch or above the workspace ceiling conflicts. Non-strict overprovisioning within the ceiling stays explicitly degraded; strict/exact work requires an exact trace match.
...
- **Ownership:** the runtime-neutral status writer owns checkpoint file creation and canonical writes; orchestrators emit semantic stage-completion and run-finish events that it persists.
- **Write timing:** After each stage completes successfully, the status writer MUST update the checkpoint file with the stage output.
- **Resume flow:**
...
- Handoff artifacts SHOULD tell the next session whether a `kanban-manager` sync is recommended before continuing.
...
### Runtime-Neutral Status Writer Contract
...
- Fixed args:
  ...
  - `--payload-json`: JSON string that decodes to exactly one object
- Base path rule: every call MUST include `payload_json.output_root`. Every event except compatible-run discovery via `run.resumed` MUST also include `payload_json.run_id`; discovery may omit it and MUST include the expected `orchestrator`.
- `output_root` is the base artifact root (for example `.pipeline-output`), not `<run_output_dir>`.
- Optional cross-repo anchor: when a run is executing against another repo, include `payload_json.working_project_dir`. The status writer resolves relative `output_root` against that target project directory instead of the current session worktree and always derives the checkpoint location.
- The status writer derives `<run_output_dir>` as `<output_root>/<run_id>/` and owns resolved paths, timestamps, refs, counts, active ids, and reconciliation.
- `run_id`, `task_id`, and `agent_id` MUST be safe 1–128 character basenames using letters, digits, dots, underscores, and hyphens, starting and ending with an alphanumeric character. Caller-supplied `checkpoint_path` is rejected because the writer owns that path.
...
- The neutral status writer applies child events in order and flushes canonical files once at the end.
...
- `agent.started` / `agent.heartbeat` / `agent.finished`: add `agent_id`; include `agent` on start and `task_id` only when attached to a canonical task. Instrumented Flow/Pipeline attempts include the complete ReasoningDecision as `reasoning` and update it when effective-effort `trace_evidence` becomes available.
- A Web session agent Job may use `evidence_refs` to identify bounded run-local Job and result evidence. Its AgentStatus retains the resolver's pre-dispatch `requested`, `inherited`, or `shadow` reasoning state because the current `enforced` schema requires native managed trace evidence; Job telemetry is reported separately under `REASONING_POLICY.md`. This does not waive task validation, review, cleanup, or dependency checks.
...
- The status writer MAY debounce redundant `agent.heartbeat` events within a short window when the heartbeat does not change semantic agent/task state beyond liveness timestamp.
...
#### Neutral status writer responsibilities

The neutral status writer CLI is the only component that may create or replace canonical checkpoint/status files.
...
- Serialize semantic stage/run events through `node tools/status-event.js` or the installed absolute equivalent.
...
Executors may only return semantic status deltas for the task they were assigned and their own agent attempt. The orchestrator serializes those deltas through the neutral status writer CLI; executors do not write canonical files directly.
...
- Include heartbeat deltas while the task is active when they remain useful; the writer owns canonical `updated_at` and `last_heartbeat_at` timestamps.
...
- the runtime-neutral status writer owns canonical file writes for all status entities
- orchestrator owns semantic run/task transitions
- executor owns only the live semantic attempt/task deltas it returns to the orchestrator; it never owns canonical file writes
