...
- Keep the workflow non-executing and conceptual, just as `artgen-scaffold` stays prompt/brief oriented rather than acting as a renderer.
...
The thin `ui-ux-workflow` surface may optionally export a durable bundle directly into repo-owned assets.
...
- If `ui-ux-workflow` is used without `--output-dir`, it should remain inline-only and should not create files by default.
...
- `agents/*.md` remains the source of truth for agent behavior; generated runtime or export outputs should not be hand-edited.
