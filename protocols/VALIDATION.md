...
- Workspace profiles select the normal role model/tier. The reasoning resolver selects child effort only: it never routes a raw/dynamic model, downgrades a model, or changes current/main-agent effort.
- `./protocols/capability-recovery-policy.json` and `./tools/capability-recovery.js` are the only model-uplift policy path. They permit one task-scoped tier step for `executor` or `generalist` after the same material reasoning failure repeats without meaningful progress. The profile's `recovery_ceiling_tiers` and read-only `resolve-recovery` action own raw model mapping.
- Capability recovery defaults to `off`; `shadow` never dispatches a model and `auto` requires a native model selector plus matching model/effort trace evidence. It never targets reviewer, `orchestrator-*`, the current/main agent, assurance, or an operational failure, and it never resets a repair/retry counter.
- Pipeline `auto` must atomically set task-scoped `capability_recovery_used = true` and increment `retry_opportunities_used` before the promoted spawn. Resume must preserve both fields; duplicate claims, non-sequential counts, and counts above `max_retry_rounds` are invalid. Stage 7 redispatches consume the same per-task counter.
...
- Reviewer max remains effort-only and may be selected for explicit `--review=max`, an evidence-backed high-consequence security/data-integrity review, or reviewer reasoning recovery. Generic risk, P3, wording/style preference, and optional hardening do not qualify.
...
- Local Codex workflows must attempt bounded child-trace verification with `./tools/codex-child-trace.js`, using V2's returned task path plus the Codex-injected current thread identity or a legacy spawn UUID; role mismatch, underprovisioning, and observed effort above the workspace ceiling block acceptance, while missing evidence stays unverified and cannot satisfy strict or exact work. V2 lookup fails closed without parent identity so repeated task paths cannot select stale traces. The helper reports only role/model values that match its bounded safe patterns, compares optional expected values without exposing invalid raw metadata, verifies parent-at-spawn effort without exposing the V2 task name or parent ID, and keeps equality inheritance-consistent rather than causal proof. A matching request distinct from the parent excludes simple inheritance. Trace discovery rejects symlinked or redirected ancestors, files, junctions, and reparse-like paths.
...
- Reasoning-effort recovery carries the prior attempt's effective class and normally runs before model capability recovery. A deep material retry may receive automatic `max` through `recovery_boost`; it is not an `explicit_effort` override. The sole earlier-uplift exception is an exact verified `lsa-efficiency-v2` execution stage admitted by the shared resolver from repeated material Sol failure evidence; it consumes the same retry/uplift budget and requires matching trace evidence.
