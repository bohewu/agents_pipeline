...
- Local Codex workflows must attempt bounded child-trace verification with `./tools/codex-child-trace.js`, using V2's returned task path plus the Codex-injected current thread identity or a legacy spawn UUID; role mismatch, underprovisioning, and observed effort above the workspace ceiling block acceptance, while missing evidence stays unverified and cannot satisfy strict or exact work. V2 lookup fails closed without parent identity so repeated task paths cannot select stale traces. The helper compares an optional bounded expected model without emitting mismatched raw model metadata, verifies parent-at-spawn effort without exposing raw role metadata, the V2 task name, or the parent ID, and keeps equality inheritance-consistent rather than causal proof. A matching request distinct from the parent excludes simple inheritance. Trace discovery rejects symlinked or redirected ancestors, files, junctions, and reparse-like paths.
