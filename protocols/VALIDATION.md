...
- Reasoning-effort recovery carries the prior attempt's effective class and normally runs before model capability recovery. A deep material retry may receive automatic `max` through `recovery_boost`; it is not an `explicit_effort` override. The sole earlier-uplift exception is an exact verified `openai-gpt6-v1` execution stage admitted by the shared resolver from repeated material Sol failure evidence; it consumes the same retry/uplift budget and requires matching trace evidence.
