...
function Test-ReleaseBundle {
    ...
    $requiredPaths = @(
        ...
        (Join-Path $BundleDir "tools/reasoning-policy.js"),
        (Join-Path $BundleDir "tools/reasoning-vocabulary.js"),
        (Join-Path $BundleDir "protocols"),
        (Join-Path $BundleDir "protocols/reasoning-policy.json"),
...
$headers = @{
    ...
    "User-Agent" = "agents-pipeline-bootstrap-claude"
}
...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("agents-pipeline-bootstrap-claude-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    $installScript = Join-Path $bundleDir "scripts/install-claude.ps1"
