...
Usage:
  scripts/bootstrap-install-codex.sh [--repo <owner/repo>] [--version <tag|latest>] [--target <path>] [--no-backup] [--force] [--dry-run] [--keep-temp]
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/agents-pipeline-bootstrap-codex.XXXXXX")"
...
INSTALL_SCRIPT="${BUNDLE_DIR}/scripts/install-codex.sh"
