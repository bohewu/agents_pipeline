...
$headers = @{
    ...
    "User-Agent" = "agents-pipeline-bootstrap-plugin-status-runtime"
}
...
Write-Host "Install scope: OpenCode status plugin only"
if ($Target) {
...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("agents-pipeline-bootstrap-plugin-status-runtime-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    $installScript = Join-Path $bundleDir "scripts/install-plugin-status-runtime.ps1"
