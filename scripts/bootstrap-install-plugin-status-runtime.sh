...
usage() {
  cat <<'EOF'
Download a release bundle and run install-plugin-status-runtime.sh without cloning this repository.

Usage:
  scripts/bootstrap-install-plugin-status-runtime.sh [--repo <owner/repo>] [--version <tag|latest>] [--target <path>] [--no-backup] [--dry-run] [--keep-temp] [--verbose]
...
Note: This bootstrap path installs the OpenCode status plugin only.
EOF
}
...
echo "Install scope: OpenCode status plugin only"
if [[ -n "${TARGET}" ]]; then
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/agents-pipeline-bootstrap-plugin-status-runtime.XXXXXX")"
...
INSTALL_SCRIPT="${BUNDLE_DIR}/scripts/install-plugin-status-runtime.sh"
