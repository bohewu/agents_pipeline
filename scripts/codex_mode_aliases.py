...
AD_HOC_MANAGED_ROLE_DISPATCH_TEMPLATE = (
    ...
    "workflow, read the `Ad-hoc managed-role dispatch` section of `{reasoning_policy}` "
    "before spawning. Keep this path adaptive, pass tier `unknown` when the profile/runtime "
    "cannot prove it, and create no workflow artifacts; unverifiable or unhealthy profile "
    "status and resolver or selector conflicts stop dispatch."
)
MATERIALITY_AND_GOAL_TEMPLATE = (
    "Before resuming a run, editing after a failed check, or starting any repair, "
    "reviewer followup, capability recovery, or new Goal continuation round, read "
    "`{materiality_gate}`. "
    "Apply its requirement-authority, failure-classification, validation-infrastructure, "
    "budget, stop, and resume/strategy-delta rules; only material work tied to an unmet "
    "original condition may continue."
)
CAPABILITY_RECOVERY_TEMPLATE = (
    "Before capability recovery or a later recovery-stage dispatch, also read "
    "`{capability_recovery}`. Keep its effort-first default, sole qualified LSA v2 "
    "exception, eligible-role and operational-failure limits, task-scoped target "
...
VALIDATION_COST_GUARD_LINE = (
    "Keep validation bounded to the requested delivery. Workflow-generated artifacts "
...
def build_mode_summary_lines(
    ...
    support_root = support_root_ref.rstrip("/\\")
    ...
    return [
        ...
        WORKSPACE_PROFILE_PREFLIGHT_LINE,
        MODE_ALIAS_AUTHORIZATION_GUARD_LINE,
        MANAGED_CHILD_DISPATCH_TEMPLATE.format(reasoning_policy=reasoning_policy),
        AD_HOC_MANAGED_ROLE_DISPATCH_TEMPLATE.format(
