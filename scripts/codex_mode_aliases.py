...
MATERIALITY_AND_GOAL_LINE = (
    ...
    "material reasoning retry, normally run reasoning-effort recovery before model "
    "capability recovery and carry the prior attempt's `effective_class` as the next "
    "retry floor. The sole earlier-uplift exception is an exact verified LSA v2 "
    "execution stage admitted from repeated material Sol failure evidence; it uses "
    "the same retry/uplift budget and matching trace evidence. Otherwise, a deep child "
    "may automatically receive `max` through `recovery_boost`, not `explicit_effort`. "
    "A verified LSA continuation retains its already approved task-scoped target "
    "binding for later stage efforts without requesting another uplift; changing "
    "recovery mode to off or shadow does not downgrade the task or refund counters. "
    "A Goal continuation must prefer same-run resume, then a "
