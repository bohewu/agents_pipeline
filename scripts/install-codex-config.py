...
from codex_mode_aliases import RUN_COMMAND_ONLY_MODE_ALIASES, build_mode_summary_lines
...
def build_agents_alias_map_lines(commands_dir: Path) -> List[str]:
    ...
    for command_name, agent_name in ordered_run_command_items(command_agents):
        ...
        if alias in RUN_COMMAND_ONLY_MODE_ALIASES:
            lines.append(f"- `{command_name}` -> `{agent_name}`")
        else:
