...
usage() {
  cat <<'EOF'
Install the local OpenCode usage status plugin from this repository.

Usage:
  scripts/install-plugin-usage-status.sh [--target <path>] [--dry-run] [--no-backup]

Options:
  --target <path>  Install plugin entry file (default: $XDG_CONFIG_HOME/opencode/plugins/usage-status.js or ~/.config/opencode/plugins/usage-status.js)
  --dry-run        Print actions without writing files
  --no-backup      Skip backup of existing usage-status plugin files
...
SOURCE_ENTRY_FILE="${REPO_ROOT}/opencode/plugins/usage-status.js"
SOURCE_SUPPORT_DIR="${REPO_ROOT}/opencode/plugins/usage-status"
...
if [[ -n "${XDG_CONFIG_HOME:-}" ]]; then
  TARGET_FILE="${XDG_CONFIG_HOME}/opencode/plugins/usage-status.js"
else
  TARGET_FILE="${HOME}/.config/opencode/plugins/usage-status.js"
fi
...
RELATIVE_SPEC="./plugins/usage-status/index.js"
