...
`adaptive` requests explicit effort on the selected dispatch surface. Native
managed children follow the local Codex trace-verification contract; Web
session agent Jobs follow the separate Job-evidence contract in
`protocols/REASONING_POLICY.md`. Neither a selector request nor a Job launch
alone is enforcement evidence.
...
native managed normal dispatch omits a raw model; a Web session agent Job selects the saved role model under `protocols/REASONING_POLICY.md`. A pinned or retired catalog requires explicit
...
4. Obey all selected workflow hard constraints, delegation, task bounds, verification, cleanup, status, reasoning, and final-report requirements. Resolve every dispatched role through the formal shared resolver as intent -> class -> selected capability -> effort; never change the current/main agent. Native managed dispatch uses effective Codex role routing and normally omits a model override. Web session agent Jobs explicitly select the saved role model, apply effort only in adaptive mode as required by `protocols/REASONING_POLICY.md` and must not claim native role identity. The sole native managed recovery exception is a Flow/Pipeline child recovery that fully satisfies `protocols/CAPABILITY_RECOVERY.md`: its one recovery spawn may pass the profile-resolved raw model and must verify that model and effort by child trace before acceptance. Other runtime exports conflict rather than inventing model routing; shadow may only compute a proven tier policy without spawning. In the final response, match the user's language and translate internal agent/protocol output into ordinary engineering language unless protocol details were requested.
