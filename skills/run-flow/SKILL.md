...
6. Obey its task bound, delegation, verification, review, cleanup, reasoning, and final-report constraints. Effective Codex configuration selects actual role models/tiers; policy v2 resolves each child as intent -> class -> selected capability -> effort, never changing the current/main agent or passing a model override. Policy v2 defaults to inherit; explicit adaptive mode requests the selector and verifies the local child trace before claiming the effective-effort contract was enforced. Child/parent equality remains selector-causality indeterminate. `--review=max` is exact reviewer-only effort: adaptive requests and verifies it, shadow records it without applying it, and inherit conflicts. It remains deep ordinary review and never applies to a non-review role.
