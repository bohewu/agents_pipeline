...
4. Let the effective workspace/global Codex agent configuration select profile-specific role files and normal models/tiers. Resolve each child through `protocols/REASONING_POLICY.md` and select registered roles without full-history forks; the reasoning resolver never changes the current/main agent or routes a raw/dynamic model. Normally omit `model`. The sole managed exception is one `executor`/`generalist` auto recovery admitted by `protocols/CAPABILITY_RECOVERY.md`, using only the active profile's read-only resolution and matching model/effort trace evidence. Adaptive requests the effort selector and verifies the local child trace before claiming the contract was enforced; child/parent equality remains selector-causality indeterminate. When the definition parses `--review=max`, pass the exact reviewer-only effort override: adaptive requests and verifies it, shadow records it without applying it, and inherit conflicts. It remains ordinary deep review and does not apply to a non-review role. Do not rewrite role files.
