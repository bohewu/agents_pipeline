...
6. Obey its scope, delegation, verification, cleanup, reasoning, and final-report constraints. Effective Codex configuration selects actual role models/tiers; the formal shared resolver resolves each child as intent -> class -> selected capability -> effort, never changing the current/main agent or passing a model override. Simple accepts `--capability-recovery=off|shadow|auto` only for normalization and never performs a recovery spawn. Reasoning defaults to inherit; explicit adaptive mode requests the selector and verifies the local child trace before claiming the effective-effort contract was enforced. Child/parent equality remains selector-causality indeterminate. `--review=max` is exact reviewer-only effort: adaptive requests and verifies it, shadow records it without applying it, and inherit conflicts. It remains deep ordinary review and never applies to a non-review role.
