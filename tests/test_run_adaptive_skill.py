...
class RunAdaptiveSkillContractTest(unittest.TestCase):
    ...
    def test_reasoning_policy_is_route_independent_with_adaptive_entrypoint_default(self) -> None:
        ...
        self.assertEqual(reasoning_mode["default"], "inherit")
    ...
    def test_capability_recovery_and_materiality_contracts_are_bounded(self) -> None:
        ...
        for text in (flow, pipeline):
            ...
            self.assertIn("`lsa_recovery_context`", text)
            self.assertIn("`resolveLsaRecoveryStage`", text)
            self.assertIn("canonical TaskStatus failure history", text)
            self.assertIn("`recovery_claim_id`", text)
            self.assertIn("`recovery_runtime_support`", text)
