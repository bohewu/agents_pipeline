...
function canonicalizeReasoningObservation(observation) {
  ...
  assert(observation.external_dispatch_evidence === undefined, "external_dispatch_evidence is unsupported");
  const result = {
  ...
  if (
    ...
    assert(result.trace_evidence !== undefined,
      "enforced version 3 observation requires matching trace evidence");
    assert(result.trace_evidence.trace_found
...
function canonicalizeAgentStatus(agentStatus, runConfiguration = undefined) {
  assert(agentStatus.external_dispatch_evidence === undefined, "external_dispatch_evidence is unsupported");
  const result = {
